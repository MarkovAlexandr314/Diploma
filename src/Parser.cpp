#include "Parser.h"
#include <assert.h>
#include <iostream>

void Parser::ReadCoordinates()
{

    int isize[3][3], ind_min[3], ind_max[3];
    char zonename[50];
    ZoneType_t zone_type;
    DataType_t type;
    char coordname[128];
    coords_by_zone.resize(zone_number);

    for (size_t index_zone = 0; index_zone < zone_number; index_zone++)
    {
        // Получаем тип сетки
        if (cg_zone_type(fn, index_base, index_zone + 1, &zone_type))
            cg_error_exit();
        // Пока обрабатываем только структурированные сетки
        if (zone_type != Structured)
            assert("Structured zone expected!!!");
        // Читаем размеры и имя зоны
        if (cg_zone_read(fn, index_base, index_zone + 1, zonename, isize[0]))
            cg_error_exit();

        // Минимальный индекс в зоне
        ind_min[COORD::X] = 1;
        ind_min[COORD::Y] = 1;
        ind_min[COORD::Z] = 1;

        // Максимальный индекс в зоне
        ind_max[COORD::X] = isize[0][COORD::X];
        ind_max[COORD::Y] = isize[0][COORD::Y];
        ind_max[COORD::Z] = isize[0][COORD::Z];

        // Аллоцируем необходимое место в памяти
        // TODO: Проверить можно ли эффективнее распределять это дело в памяти
        coords_by_zone[index_zone].resize(ind_max[COORD::X]);
        for (size_t i{0}; i < ind_max[COORD::X]; i++)
        {
            coords_by_zone[index_zone][i].resize(ind_max[COORD::Y]);
            for (size_t j{0}; j < ind_max[COORD::Y]; j++)
            {
                coords_by_zone[index_zone][i][j].resize(ind_max[COORD::Z]);
                for (size_t k{0}; k < ind_max[COORD::Z]; k++)
                {
                    coords_by_zone[index_zone][i][j][k].resize(dim_number);
                }
            }
        }

        // Читаем кооринаты сетки
        std::vector<float> temp(ind_max[COORD::X] * ind_max[COORD::Y] * ind_max[COORD::Z]);
        for (size_t curr_dim = 0; curr_dim < dim_number; curr_dim++)
        {
            if (cg_coord_info(fn, index_base, index_zone + 1, curr_dim + 1, &type, coordname))
                cg_error_exit();
            if (cg_coord_read(fn, index_base, index_zone + 1, coordname, type, ind_min, ind_max, &temp[0]))
                cg_error_exit();
            const char *dtype_str = "?";
            switch (type)
            {
            case RealSingle:
                dtype_str = "RealSingle";
                break;
            case RealDouble:
                dtype_str = "RealDouble";
                break;
            }
            // std::cout
            // << "' datatype = " << dtype_str << std::endl;
            // now map temporary (1D) coordinate array to 3D coordinate array
            for (size_t vertex = 0; vertex < ind_max[COORD::X] * ind_max[COORD::Y] * ind_max[COORD::Z]; ++vertex)
            {
                size_t i = vertex % ind_max[COORD::X];
                size_t j = vertex % ind_max[COORD::Y];
                size_t k = vertex / (ind_max[COORD::X] * ind_max[COORD::Y]);

                coords_by_zone[index_zone][i][j][k][curr_dim] = temp[vertex];
            }
        }
        std::cout << coords_by_zone[index_zone].size() << std::endl;
    }
}

void Parser::ReadInterfaces()
{

    int nzones;
    cg_nzones(fn, 1, &nzones);

    int CGNS_MAX_NAME_LENGTH{40};
    int numberInterfaces{0};

    // Наши сетки пока хранят только интерфесы каждая для себя
    // TODO: Расширить функционал
    // int n1to1;
    int itranfrm[3];
    int n1to1;
    int index_file, nzone, index_zone, index_conn;
    char donorname[33], connectname[33];
    cgsize_t ipnts[2][3], ipntsdonor[2][3];
    for (size_t index_zone = 0; index_zone < zone_number; index_zone++)
    {
        // Получаем число интерфейсов для данной зоны
        std::cout << index_base << ' ' << index_zone + 1 << ' ' << n1to1 << std::endl;
        if (cg_n1to1(fn, index_base, index_zone + 1, &n1to1))
            cg_error_exit();
        std::cout << index_zone + 1 << ' ' << n1to1 << std::endl;
        interface_connectivity[index_zone].resize(n1to1);

        for (size_t index_conn = 0; index_conn < n1to1; index_conn++)
        {
            // Читаем каждый интерфейс в зоне
            if (cg_1to1_read(fn, index_base, index_zone + 1, index_conn + 1, connectname, donorname,
                             ipnts[0], ipntsdonor[0], itranfrm))
                cg_error_exit();

            // Проверка: записан ли в Transform [0, 0, 0]
            // Если записан меняем [0, 0, 0] -> [1, 2, 3]
            if (!itranfrm[0] && !itranfrm[1] && !itranfrm[2])
            {
                itranfrm[0] = 1;
                itranfrm[1] = 2;
                itranfrm[2] = 3;
            }

            std::vector<std::vector<int>> tMatrix(dim_number, std::vector<int>(dim_number));
            for (int dimension = 0; dimension < dim_number; ++dimension)
            {
                int value = itranfrm[dimension];
                tMatrix[std::abs(value) - 1][dimension] = (value > 0) ? 1 : ((value < 0) ? -1 : 0);
            }

            // Считаем случай: стыковка по некоторой грани(одна из компонент i, j, k = const)
            size_t numberOfIndicesX = std::abs((int)ipnts[0][COORD::X] - (int)ipnts[1][COORD::X]);
            size_t numberOfIndicesY = std::abs((int)ipnts[0][COORD::Y] - (int)ipnts[1][COORD::Y]);
            size_t numberOfIndicesZ = std::abs((int)ipnts[0][COORD::Z] - (int)ipnts[1][COORD::Z]);
            size_t numberOfIndices = std::max(std::max(numberOfIndicesX + 1, numberOfIndicesY + 1),
                                              numberOfIndicesZ + 1);

            interface_connectivity[index_zone][index_conn].ownerIndex.resize(numberOfIndices);
            interface_connectivity[index_zone][index_conn].donorIndex.resize(numberOfIndices);

            for (size_t index = 0; index < numberOfIndices; ++index)

            {
                const size_t ownerStartX = (int)ipnts[0][COORD::X] - 1;
                const size_t ownerStartY = (int)ipnts[0][COORD::Y] - 1;
                const size_t ownerStartZ = (int)ipnts[0][COORD::Z] - 1;

                const size_t neighbourStartX = (int)ipntsdonor[0][COORD::X] - 1;
                const size_t neighbourStartY = (int)ipntsdonor[0][COORD::Y] - 1;
                const size_t neighbourStartZ = (int)ipntsdonor[0][COORD::Z] - 1;

                const size_t indexX = numberOfIndicesX == 0 ? 0 : index;
                const size_t indexY = numberOfIndicesY == 0 ? 0 : index;
                const size_t indexZ = numberOfIndicesZ == 0 ? 0 : index;

                size_t ownerIndexX = ownerStartX + indexX;
                size_t ownerIndexY = ownerStartY + indexY;
                size_t ownerIndexZ = ownerStartZ + indexZ;

                size_t neighbourIndexX = tMatrix[0][0] * indexX + tMatrix[0][1] * indexY + tMatrix[0][2] * indexZ + neighbourStartX;
                size_t neighbourIndexY = tMatrix[1][0] * indexX + tMatrix[1][1] * indexY + tMatrix[1][2] * indexZ + neighbourStartY;
                size_t neighbourIndexZ = tMatrix[2][0] * indexX + tMatrix[2][1] * indexY + tMatrix[2][2] * indexZ + neighbourStartZ;

                interface_connectivity[index_zone][index_conn].donor_zone_name = donorname;
                interface_connectivity[index_zone][index_conn].ownerIndex[index][COORD::X] = ownerIndexX;
                interface_connectivity[index_zone][index_conn].ownerIndex[index][COORD::Y] = ownerIndexY;
                interface_connectivity[index_zone][index_conn].ownerIndex[index][COORD::Z] = ownerIndexZ;

                interface_connectivity[index_zone][index_conn].donorIndex[index][COORD::X] = neighbourIndexX;
                interface_connectivity[index_zone][index_conn].donorIndex[index][COORD::Y] = neighbourIndexY;
                interface_connectivity[index_zone][index_conn].donorIndex[index][COORD::Z] = neighbourIndexZ;
            }
        }
    }
}

void Parser::ReadSolution()
{
    char solname[33], zonename[33], coordname[33], fieldname[33];
    int isize[3][3], ind_min[3], ind_max[3], nfields{0}, nsols{0};
    DataType_t type;

    flow_solution.resize(zone_number);
    flow_solution_map.resize(zone_number);

    // Обхожим зоны
    for (size_t index_zone = 0; index_zone < zone_number; index_zone++)
    {
        // Получаем, сколько решений
        if (cg_nsols(fn, index_base, index_zone + 1, &nsols))
            cg_error_exit();

        flow_solution[index_zone].resize(nsols);
        flow_solution_map[index_zone].resize(nsols);

        // Обходим solutions
        for (size_t index_sol = 0; index_sol < nsols; index_sol++)
        {
            // Получаем число полей
            if (cg_nfields(fn, index_base, index_zone + 1, index_sol + 1, &nfields))
                cg_error_exit();
            flow_solution[index_zone][index_sol].resize(nfields);
            flow_solution_map[index_zone][index_sol].resize(nfields);

            // Обходим поля
            for (size_t index_flow = 0; index_flow < nfields; index_flow++)
            {
                if (cg_field_info(fn, index_base, index_zone + 1, index_sol + 1, index_flow + 1, &type, fieldname))
                    cg_error_exit();

                std::cout << "========== FIELDNAME ============ " << fieldname << std::endl;
                flow_solution_map[index_zone][index_sol][index_flow] = std::string(fieldname);
                if (cg_zone_read(fn, index_base, index_zone + 1, zonename, isize[0]))
                    cg_error_exit();

                ind_min[COORD::X] = 1;
                ind_min[COORD::Y] = 1;
                ind_min[COORD::Z] = 1;

                // Максимальный индекс в зоне
                ind_max[COORD::X] = isize[1][COORD::X];
                ind_max[COORD::Y] = isize[1][COORD::Y];
                ind_max[COORD::Z] = isize[1][COORD::Z];

                // Выделяем достаточное место под i, j, k
                flow_solution[index_zone][index_sol][index_flow].resize(ind_max[COORD::X]);
                for (size_t i{0}; i < ind_max[COORD::X]; i++)
                {
                    flow_solution[index_zone][index_sol][index_flow][i].resize(ind_max[COORD::Y]);
                    for (size_t j{0}; j < ind_max[COORD::Y]; j++)
                    {
                        flow_solution[index_zone][index_sol][index_flow][i][j].resize(ind_max[COORD::Z]);
                    }
                }

                // Временный массив
                std::vector<float> temp(ind_max[COORD::X] * ind_max[COORD::Y] * ind_max[COORD::Z]);

                // Читаем решение потока
                if (cg_field_read(fn, index_base, index_zone + 1, index_sol + 1, fieldname,
                                  type, ind_min, ind_max, temp.data()))
                    cg_error_exit();

                // Заполняем flow_solution из временного массива temp
                for (size_t vertex = 0; vertex < ind_max[COORD::X] * ind_max[COORD::Y] * ind_max[COORD::Z]; ++vertex)
                {
                    size_t i = vertex % ind_max[COORD::X];
                    size_t j = vertex % ind_max[COORD::Y];
                    size_t k = vertex / (ind_max[COORD::X] * ind_max[COORD::Y]);

                    flow_solution[index_zone][index_sol][index_flow][i][j][k] = temp[vertex];
                }
            }
        }
    }
}

FlowSolutionMap_Type Parser::GetFlowSolMap() const
{

    return flow_solution_map;
}

FlowSolution3D_Type Parser::GetFlowSolution() const
{
    return flow_solution;
}

int Parser::GetZoneNumber() const
{
    return zone_number;
}

Vector3D_Type Parser::GetCoordinates() const
{
    return coords_by_zone;
}

// Получить интерфейс связностей
InterfaceConnectivity_Type Parser::GetInterfaceConnectivity() const
{
    return interface_connectivity;
}

std::unordered_map<std::string, int> Parser::GetZoneNameMap() const
{
    return zone_name_map;
}

Parser::Parser(const std::string &filename_)
{
    filename = filename_;
    char zonename[50];
    int isize[3][3];

    // Открываем файл на чтение
    if (cg_open(filename.c_str(), CG_MODE_READ, &fn))
        cg_error_exit();

    // Получаем кол-во зон
    if (cg_nzones(fn, index_base, &zone_number))
        cg_error_exit();

    // Сопоставляем пары: имя зоны -> индекс
    for (size_t index_zone = 0; index_zone < zone_number; index_zone++)
    {
        if (cg_zone_read(fn, index_base, index_zone + 1, zonename, isize[0]))
            cg_error_exit();

        zone_name_map[zonename] = index_zone + 1;
    }
}

Parser::~Parser()
{
    cg_close(fn);
}
